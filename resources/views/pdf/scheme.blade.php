...
<head>
    ...
    <style>
        ...
        td {
            ...
            /* Light gray from palette */
            text-align: left;
        }

        th {
            background: #dde3eb;
            /* Soft fawn/gray background */
            width: 35%;
            color: #10414b;
        }
...
<body>
    ...
    <table>
        ...
        @endif
        <tr>
            <th>Donors / Collaborators</th>
            <td>
                @if(!empty($scheme->collaborator))
                    @php
                        $donorNames = \App\Models\Donor::whereIn('id', $scheme->collaborator)->pluck('name')->toArray();
                    @endphp
                    {{ implode(', ', $donorNames) }}
                @else
                    N/A
    ...
    <footer>
        Generated on {{ now()->format('d M Y, h:i A') }} | © HELVETAS Nepal WSMIS
    </footer>
